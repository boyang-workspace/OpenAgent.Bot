...
import { getConfiguredLocalModel } from "./app-settings";
...
const promptDefaults: Record<PromptVersionKind, { version: string; content: string; config: Record<string, unknown> }> = {
  ...
  "draft-generation": {
    version: "v2",
    content: [
      "Write a decision-oriented OpenAgent draft for a search user who wants to compare real tools and make a next-step decision.",
      "Lead with a concrete recommendation, name the actual products being compared, and explain tradeoffs instead of writing generic filler.",
      "Use natural SEO phrasing for the primary keyword in the intro and relevant headings, but do not stuff keywords.",
      "Every claim about a tool must stay grounded in the provided source links. If evidence is thin, say what still needs verification."
    ].join("\n\n"),
...
function sourceLinksForTopic(topic: Pick<BlogTopic, "sourceSignals">): string[] {
  return [...new Set([...topic.sourceSignals.filter((link) => /^https?:/i.test(link)), ...editorialSourceDefaults])].slice(0, 3);
}
...
function draftPrompt(
  ...
  return [
    ...
    "The article must sound like an editorial comparison, not a placeholder template.",
    "The intro should tell the reader who this comparison is for, what decision they are making, and what you recommend they evaluate first.",
    "Name concrete browser-agent products or repositories when the evidence supports it.",
    "The body must include at least one markdown comparison table and visible source links.",
    "The body must include at least 3 internal OpenAgent links: /agents, /memory-systems, /skills.",
    "The FAQ must contain at least 3 H3 questions with direct answers.",
    "Do not mention that the article is auto-generated, a draft, or needs manual review inside the body.",
    `Title: ${topic.title}`,
    `Angle: ${topic.angle}`,
...
function repairDraftPrompt(
  topic: Pick<BlogTopic, "title" | "date" | "lane" | "angle" | "primaryKeyword" | "searchIntent" | "sourceSignals">,
  draft: BlogPostContent,
  promptContent: string,
  issues: string[]
): string {
  const sourceLinks = sourceLinksForTopic(topic);
  return [
    promptContent,
    "",
    "You are repairing a local-model draft so it passes a hard editorial quality gate.",
    "Return strict JSON with shape:",
    "{\"summary\": string, \"seoTitle\": string, \"seoDescription\": string, \"body\": string}",
    "Preserve the topic, editorial angle, and useful parts of the draft, but rewrite whatever is necessary to satisfy every issue below.",
    `Primary keyword: ${topic.primaryKeyword}`,
    `Search intent: ${topic.searchIntent}`,
    `Allowed source links: ${sourceLinks.join(", ")}`,
    `Hard requirements to fix: ${issues.join(" | ")}`,
    "The body must include at least one markdown comparison table, at least 3 internal OpenAgent links, and an FAQ section with H3 questions.",
    "Current draft body:",
    draft.body
  ].join("\n");
}

function draftFromLocalResponse(
  topic: Pick<BlogTopic, "title" | "date" | "lane" | "angle" | "primaryKeyword" | "searchIntent">,
  parsed: { summary?: string; seoTitle?: string; seoDescription?: string; body?: string } | undefined
): BlogPostContent {
  return {
    slug: topic.title.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, ""),
    title: topic.title,
    summary: parsed?.summary?.trim() || topic.angle,
    publishedAt: topic.date,
    tags: [topic.lane, "open-source", "ai-agents"],
    author: "OpenAgent.bot Editors",
    body: parsed?.body?.trim() || "",
    seoTitle: (parsed?.seoTitle?.trim() || `${topic.title} | OpenAgent.bot`).slice(0, 80),
    seoDescription: (parsed?.seoDescription?.trim() || topic.searchIntent).slice(0, 180)
  };
}
...
export async function generateDraftFromTopicWithLocalModel(
  ...
  const model = options.model ?? (await getConfiguredLocalModel(env)) ?? defaultLocalModel(env);
  const sourceLinks = sourceLinksForTopic(topic);
  ...
  let draft = draftFromLocalResponse(topic, local.parsed);
  let payload = parseBlogDraftPayload({
    ...draft,
  ...
  if (!payload.qualityReport.passed) {
    const repaired = await runLocalModelJson<{ summary: string; seoTitle: string; seoDescription: string; body: string }>(env, {
      model,
      temperature: 0.1,
      prompt: repairDraftPrompt(topic, draft, promptContent, payload.qualityReport.issues)
    });
    draft = draftFromLocalResponse(topic, repaired.parsed);
    payload = parseBlogDraftPayload({
      ...draft,
      targetKeyword: topic.primaryKeyword,
      searchIntent: topic.searchIntent,
      sourceLinks
    ...
    repairAttempts.push({
      ...
      qualityIssues: payload.qualityReport.issues
    });
    if (!payload.qualityReport.passed) {
      throw new Error(`Local model draft failed quality gate after repair: ${payload.qualityReport.issues.join(" ")}`);
    }
  }
...
export async function generateOutlineWithLocalModel(
  ...
  const local = await runLocalModelJson<{ title: string; sections: string[] }>(env, {
    model: options.model ?? (await getConfiguredLocalModel(env)) ?? defaultLocalModel(env),
    temperature: 0.1,
