...
export const onRequestPost: PagesFunction<Env> = async ({ request, env, params }) => {
  ...
  try {
    ...
    const content: ProjectDraftContent = {
      ...project,
      status: "archived",
      reviewedAt: now,
    ...
    if (dryRun) {
      return ok({ dryRun: true, operation: "archive", sourceFilePath: project.filePath, content });
    }
    ...
    if (existing) {
      ...
      await env.DB.prepare(
        `UPDATE project_drafts
         SET title = ?, category = ?, status = 'ready', operation = 'archive', content_json = ?, source_file_path = ?,
        ...
        .run();
      await logEvent(env, "project_draft", existingDraft.id, "created_archive_draft", { actor, before: existingDraft.content, after: content });
