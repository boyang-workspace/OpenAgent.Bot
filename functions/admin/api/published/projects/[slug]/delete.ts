...
export const onRequestPost: PagesFunction<Env> = async ({ request, env, params }) => {
  ...
  try {
    ...
    if (dryRun) {
      return ok({ dryRun: true, operation: "delete", sourceFilePath: project.filePath, content });
    }
    ...
    if (existing) {
      ...
      await env.DB.prepare(
        `UPDATE project_drafts
         SET title = ?, category = ?, status = 'ready', operation = 'delete', content_json = ?, source_file_path = ?,
        ...
        .run();
      await logEvent(env, "project_draft", existingDraft.id, "created_delete_draft", { actor, before: existingDraft.content, after: content });
