...
Daily blog automation is draft-first. It creates topic candidates and blog drafts, then leaves publishing to Admin review.

Local preview:

```bash
npm run blog:daily -- --dry-run --limit 1
```

Run and write review files:

```bash
npm run blog:daily -- --limit 1
```

If `OPENAGENT_BLOG_IMPORT_URL` points to `/admin/api/blog/drafts`, accepted drafts are imported into D1 instead of written to `content/blog/drafts`.

Admin blog workflow:

1. Open `/admin/blog`.
2. Generate or review topic candidates.
3. Create a draft from a topic.
4. Edit title, slug, SEO fields, source links, and body.
5. Mark ready only after the quality gate passes.
6. Create a GitHub PR. The PR writes `content/blog/published/<slug>.json`, and `/blog/<slug>` appears after deploy.

Local workbench additions:

1. `/admin/blog` now supports manual topic creation, priority changes, duplicate warnings, editable drafts, review reruns, and explicit human approval.
2. `/admin/blog-lab` manages prompt versions, outline templates, and sandbox debug runs.
3. Local model generation and review use Ollama via `OLLAMA_BASE_URL` and `OLLAMA_DEFAULT_MODEL`.
4. If Ollama is unavailable, draft/debug actions fall back to deterministic seed/template behavior instead of failing the whole flow.
