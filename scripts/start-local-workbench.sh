...
pick_model() {
  ...
  for candidate in qwen3.5:9b gemma4:e4b qwen3.5:4b qwen3.5:2b; do
...
run_local_migrations() {
  local log_file="/tmp/openagent-d1-migrations.log"
  rm -f "$log_file"

  npm run d1:migrations:local >"$log_file" 2>&1 &
  local pid="$!"
  ...
  for _ in {1..25}; do
    ...
    if grep -q "No migrations to apply\\|Successfully applied\\|Migrations applied" "$log_file" 2>/dev/null; then
      cat "$log_file" >&2
  ...
  kill "$pid" >/dev/null 2>&1 || true
  wait "$pid" >/dev/null 2>&1 || true
  fail "Local D1 migrations did not finish in time."
}
