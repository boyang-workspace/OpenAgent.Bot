...
function linkUrl(resource: ResourceV1, type: ResourceLink["type"]): string | undefined {
  return resource.links.items.find((item) => item.type === type)?.url;
}

function openSourceStatus(resource: ResourceV1): OpenSourceStatus {
  if (resource.decision_signals.open_source) return "open-source";
  if (resource.facts.pricing_model === "unknown") return "unknown";
  return "source-available";
}

export function resourceV1ToOpenProject(resource: ResourceV1): OpenProject {
  const category = resource.classification.primary_category as CategorySlug;
  const repoUrl = linkUrl(resource, "github");
  const homepageUrl = linkUrl(resource, "homepage");
  const docsUrl = linkUrl(resource, "docs");
  const demoUrl = linkUrl(resource, "demo");
  const allTags = uniq([
  ...
  const sourceLinks = uniq(resource.links.items.map((item) => item.url));
  const updatedAt = resource.timestamps.updated_at.slice(0, 10);
  const reviewedAt = resource.timestamps.published_at ?? resource.facts.last_verified_at ?? resource.timestamps.updated_at;

  return {
    slug: resource.slug,
    title: resource.identity.name,
    oneLiner: resource.identity.one_liner,
    summary: resource.identity.short_description ?? resource.identity.one_liner,
    whyItMatters: resource.positioning.why_it_matters ?? resource.editorial?.seo_article?.why_it_matters ?? resource.identity.one_liner,
    bestFor: resource.positioning.best_for ?? [],
    notFor: resource.positioning.not_for,
    category,
    tags: allTags,
    repoUrl,
    homepageUrl,
    ...
    maintainer: resource.facts.github_repo_full_name?.split("/")[0],
    worksWith: resource.capabilities.integrations,
    sourceLinks: sourceLinks.length ? sourceLinks : [resource.links.primary_url],
    seoTitle: resource.seo?.title ?? `${resource.identity.name}: Open-source ${category.replaceAll("-", " ")} for AI builders`,
    seoDescription: resource.seo?.description ?? resource.identity.short_description ?? resource.identity.one_liner,
    shareTitle: `${resource.identity.name} on OpenAgent.bot`,
    shareDescription: resource.identity.one_liner,
    status: resource.status === "published" ? "published" : "archived",
    generatedAt: resource.timestamps.created_at,
    reviewedAt,
    updatedAt,
    openSourceStatus: openSourceStatus(resource),
    isFeatured: Boolean(resource.editorial?.featured_reason),
    isSponsored: false,
    featuredReason: resource.editorial?.featured_reason,
    coverImage: resource.media.thumbnail_url ?? resource.media.og_image_url,
    coreStrengths: resource.editorial?.core_strengths?.map((item) => ({
      title: item.title,
      description: item.description,
      whyItMatters: item.why_it_matters
    })),
    useCaseNotes: resource.editorial?.use_case_notes,
    compareNotes: resource.editorial?.compare_notes,
    gettingStarted: resource.editorial?.getting_started,
    commandLine: resource.editorial?.command_line,
    seoArticle: resource.editorial?.seo_article
      ? {
          intro: resource.editorial.seo_article.intro,
          whatItIs: resource.editorial.seo_article.what_it_is,
          whyItMatters: resource.editorial.seo_article.why_it_matters,
          howItWorks: resource.editorial.seo_article.how_it_works,
          useCases: resource.editorial.seo_article.use_cases,
          alternatives: resource.editorial.seo_article.alternatives,
          gettingStarted: resource.editorial.seo_article.getting_started,
          faq: resource.editorial.seo_article.faq
        }
      : undefined,
    thumbnailBrief: resource.media.thumbnail_brief
      ? {
          resourceType: resource.media.thumbnail_brief.resource_type,
          visualMotif: resource.media.thumbnail_brief.visual_motif,
          backgroundStyle: resource.media.thumbnail_brief.background_style,
          titleOverlay: resource.media.thumbnail_brief.title_overlay,
          subtitle: resource.media.thumbnail_brief.subtitle,
          priorityAssets: resource.media.thumbnail_brief.priority_assets,
          avoid: resource.media.thumbnail_brief.avoid
        }
      : undefined,
    sourceMetrics: {
      stars: resource.facts.github_stars,
      forks: resource.facts.github_forks
    },
